          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['Help Center', 'Safety Resources', 'Crisis Hotlines', 'Report Abuse'].map(item => (
                <li key={item}>
                  <Link to="/safety" className="text-sm text-white/50 hover:text-white transition-colors font-body">{item}</Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-6">
            {['Privacy', 'Terms', 'Cookies'].map(item => (
              <span key={item} className="text-xs text-white/30 hover:text-white/60 transition-colors cursor-pointer font-mono">{item}</span>
            ))}
          </div>
        </div>
      </div>
